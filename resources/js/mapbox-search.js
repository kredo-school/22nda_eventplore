...
if (firstEvent) {
    const firstEventCoordinates = [firstEvent.longitude, firstEvent.latitude];

    // 画面の右上方に調整するためのオフセットを設定
    const offsetLongitude = -0.3; // 調整するオフセット（経度）
    const offsetLatitude = -0.1; // 調整するオフセット（緯度）

    // 中心を設定
    map.on('load', () => {
        // 新しい中心位置を設定
        const newCenter = [firstEventCoordinates[0] + offsetLongitude, firstEventCoordinates[1] + offsetLatitude];
